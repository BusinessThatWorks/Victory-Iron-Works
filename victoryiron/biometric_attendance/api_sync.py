...
def _find_employee(employee_id: str | None, employee_name: str | None):
	"""
	Find employee by attendance_device_id (Employee_ID from API) first, then fallback to name.
	attendance_device_id is unique and stored in attendance_device_id field.
	"""
	emp = None

	# Primary match: Use attendance_device_id (Employee_ID from API)
	# This is stored in the attendance_device_id field and is unique
	if employee_id:
		emp = frappe.db.get_value(
			"Employee",
			{"status": ["in", ["Active", "Onboarding"]], "attendance_device_id": employee_id},
			["name", "employee_name", "attendance_device_id", "holiday_list"],
			as_dict=True,
		)
		if emp:
			print(f"✅ Matched by attendance_device_id: {employee_id} -> {emp.employee_name}")
			return emp

	# Fallback: Try match by name (only if attendance_device_id match failed)
	if not emp and employee_name:
		...
		if emp:
			print(f"⚠️ Matched by name (no attendance_device_id match): {employee_name}")
...
def sync_attendance_range(
	start_date: str | None = None, end_date: str | None = None, employee: str | None = None
):
	...
	If employee is specified, only fetch data for that specific employee.
	"""
	...
	# Log if filtering by specific employee
	if employee:
		emp_name = frappe.db.get_value("Employee", employee, "employee_name")
		print(f"🎯 Filtering backfill for specific employee: {employee} ({emp_name})")
